...
function [wc,sum_norm_first_estimates,sum_Bregman_first_estimates,eta]=...
    distributed_mirror_descent_online_optimization(T,D,L,G,lambda,n,performance_metric,verbose,sigma)
    ...
    X(:,1) = P.MultiStartingPoints(n,equalStart); % equalStart must be set to 1

    % Starting points must be minimizers of w over the bounded set
    w_id = w + id_function; 
    xstar_w_id = w_id.OptimalPoint(); 
    P.AddConstraint((X{1,1}-xstar_w_id)^2==0);
    % Then, all X(i,1) are minimizers of w over the bounded domain.
    ...
    for t = 1:T
        for i=1:n
            ...
            P.AddConstraint((G_saved{i,t}+(1/eta)*(gw_y-gw_xit)+gid_y)^2==0);
            % ---
            Y(i,t) = {y};
        end
        AxX = A.consensus(Y(:,t));
        for i=1:n
            X(i,t+1) = AxX(i); % Communicating with its instant neighbors
        end
